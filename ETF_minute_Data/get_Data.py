...
class CpStockDataRequest:
    ...
    def RequestMinuteData(self, code, date, caller):
        ...
        self.objStockChart.SetInputValue(9, ord('0'))               # 수정주가 사용 여부
...
class StockDataController():
    ...
    def save_all_minute_data(self):
        ...
        past_date = datetime.strftime(datetime.now().date() - timedelta(days=735), "%Y%m%d")
        while True:
            try:
...
def data_validation():
    ...
    for i in range(0, len(code_list)):
        ...
        while True:
            try:
                temp = stock.get_etf_ohlcv_by_date(start_date, end_date, code[1:])
                break
            except:
                pass
